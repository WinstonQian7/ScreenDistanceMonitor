...
class FaceDetection:
    ...
    def load_face_detection():
        v = ftk.Verification()
        v.load_model("./Models/FaceDetection/20180204-160909/")
        ...
        for face in faces:
            ...
            if len(detected) > 0:
                ...
                if display_image_with_detections:
                    im_eyes = image[y:int(y + h/2), x:x + w]
                    cv2.rectangle(image, (x, y), (x + w, y + h), (255, 0, 0), 2)
                    im_face = cv2.resize(im_eyes,(300,300))

                    eyes = eye_detect.detectMultiScale(im_face, 1.05, 3)
                    for (x,y,w,h) in eyes:
                        cv2.rectangle(im_face, (x,y), (x+w, y+h), (0,0,255), 3)
